...
class Firi:
    ...
    def create_signature(
        self, endpoint: str, validity: int = 2000, extra_data: dict = None
    ) -> dict:
    ...
    def template_func(
        ...
        if query_params:
            # Query string construction for future use
            pass
    ...
    def get_transactions_by_year(self, year: int, direction: str = "end") -> dict:
        ...
        logging.info("Fetching transaction history for %s", year)
        endpoint = f"v2/history/transactions/{year}?direction={direction}"
        response = self.template_func(endpoint=endpoint, authenticated=True)

        if response is None:
            logging.error(
                f"Failed to fetch transactions for year {year}. Response is None."
            )
            return {
                "error": f"Failed to fetch transactions for year {year}. Check logs for details."
            }
        return response

    def get_transactions_by_year_month(
        self, month: int, year: int, direction: str = "end"
    ) -> dict:
        if year <= 2017:
            raise ValueError("Year must be greater than 2017.")
        ...
        response = self.template_func(
            endpoint=endpoint, authenticated=True, method="GET"
        )

        if response is None:
            logging.error(
                f"Failed to fetch transactions for year {year}, month {month}. Response is None."
            )
            return {
                "error": f"Failed to fetch transactions for year {year}, month {month}. Check logs for details."
            }
        return response
    ...
    def get_orders(self, count: int, order_type: str = None):
        ...
        """
        logging.info(
            f"Fetching order history with type={order_type} and count={count}..."
        )
        endpoint = "v2/history/orders?"
